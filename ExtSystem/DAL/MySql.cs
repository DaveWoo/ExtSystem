...
namespace DAL
{
	...
	{
		public static string MySqlConfigString { get { return "dbmysql"; } }

		public MySql(string TableName)
			: base(MySql.MySqlConfigString, TableName)
		{
		}

		public MySql(string ConfigString, string TableName)
		...
		public override bool Add(System.Data.DataTable addTable)
		{
			...
			{
				MySqlCommand sqlcmd = this.MySqlCon.CreateCommand();
				...
				if (addTable != null && addTable.Rows.Count > 0)
				{
					try
					{
						...
						}
						{
							...
							foreach (DataRow dr in addTable.Rows)
							{
								...
								{
									base.msg = "第三步 瑞典MySQL AB公司数据库增加数据成功 sql语句：" + sql;
								}
								else
								{
									base.msg = "第三步 瑞典MySQL AB公司数据库增加数据失败 sql语句：" + sql;
								}
							}
						}
					}
					...
					{
						base.msg = "第二步 瑞典MySQL AB公司数据库创建/增加数据sql 语句失败 sql语句：" + sql + "  异常信息：" + ex.Message;
					}
				}
				else
				{
		...
		{
			if (this.MySqlCon != null && this.MySqlCon.State == ConnectionState.Open)
			{
				this.MySqlCon.Dispose();
				if (this.MySqlCmd != null)
					this.MySqlCmd.Dispose();
				this.MySqlCon.Close();
				base.msg = "最后->关闭 瑞典MySQL AB公司数据库 要请继续调用对象Open() 方法连接数据库";
				return true;
			}
			return false;
		}
		...
		{
			this.MySqlCon = new MySqlConnection(sql);
			if (this.MySqlCon != null && MySqlCon.State == ConnectionState.Closed)
			{
				this.MySqlCon.Open();
		...
		public override bool Open()
		{
			try
			{
				this.MySqlCon = new MySqlConnection(base.ConfigStr);
				if (this.MySqlCon != null && MySqlCon.State == ConnectionState.Closed)
				{
					this.MySqlCon.Open();
		...
		public override DataTable GetTable(long getTableID)
		{
			...
			return this.GetTable((string.Format("select *  from  {0}    where {1}=@ID   order by {2}  limit 0, 1", base.TableName, IDAttribute, IDAttribute)),
		   keys, values);
		}
		...
		public override DataTable GetTable(List<long> getTableID)
		{
			...
			if (getTableID != null)
			{
				...
				return this.GetTable(string.Format("select *  from {0}  where  {1}  in ({4})  order by {2} limit 0, {3}", base.TableName, IDAttribute, IDAttribute, getTableID.Count, inVals),
			  null, null);
			}
		...
		public override DataTable GetTable(string getsWhere, int top, string orders)
		{
			...
			return this.GetTable(string.Format("select *  from {0}   {1}  {3}   limit 0, {2}", base.TableName, getsWhere, top, orders)
			 );
		}
		...
		public override DataTable GetTable(List<long> getTableID, string orders)
		{
			...
			if (getTableID != null)
			{
				...
				return this.GetTable(string.Format("select *  from {0}  where  {1}  in ({4})  {5} limit 0, {3}", base.TableName, IDAttribute, IDAttribute, getTableID.Count, inVals, orders),
			  null, null);
			}
		...
		public override DataTable GetTable(string getsWhere, int top)
		{
			...
			string sql = string.Format("select *  from  {0}  {1}       limit 0, {2}", base.TableName, getsWhere, top);
		...
		public override DataTable GetTable(string getsWhere, List<string> keys, List<object> values)
		{
			...
			if (this.IsOpenDataBase)
			{
				try
				{
					...
					MySqlDataReader sqr = sqlcmd.ExecuteReader();

					dtData.Load(sqr);

					if (dtData != null && dtData.Rows.Count > 0)
					{
						base.msg = "第三步 瑞典MySQL AB公司数据库读取成数据成功 sql语句：" + sql + "读取行数" + dtData.Rows.Count;
					}
					else
					{
						base.msg = "第三步 瑞典MySQL AB公司数据库读取数据成功 sql语句：" + sql + "读取行数" + dtData.Rows.Count;
						;
					}
				}
				catch (Exception ex)
		...
		public override long Del(string delWhere)
		{
...

			{
				try
				{
					...
					base.msg = "第二步 瑞典MySQL AB公司数据库创建/删除数据sql 语句成功 sql语句：" + sql + " 删除行数：" + icounter;
				}
				catch (Exception ex)
				{
					base.msg = "第二步 瑞典MySQL AB公司数据库创建/增加数据sql 语句失败 sql语句：" + sql + "  异常信息：" + ex.Message;
