...
namespace DAL
{
	...
	{
		public static string SQLITE_LOCAL_DD = "|DataDirectory|";
		public static string SQLiteConfigString { get { return "dbSQLite"; } }

		public SQLite(string TableName)
			: base(SQLite.SQLiteConfigString, TableName)
		{
		}

		public SQLite(string ConfigString, string TableName)
		...
		public override bool Add(System.Data.DataTable addTable)
		{
			...
			{
				SQLiteCommand sqlcmd = this.SQLiteCon.CreateCommand();
				...
				if (addTable != null && addTable.Rows.Count > 0)
				{
					try
					{
						...
						}
						{
							...
							foreach (DataRow dr in addTable.Rows)
							{
								...
								{
									base.msg = "第三步 SQLite，是一款轻型的数据库公司增加数据成功 sql语句：" + sql;
								}
								else
								{
									base.msg = "第三步 SQLite，是一款轻型的数据库公司增加数据失败 sql语句：" + sql;
								}
							}
						}
					}
					...
					{
						base.msg = "第二步 SQLite，是一款轻型的数据库公司创建/增加数据sql 语句失败 sql语句：" + sql + "  异常信息：" + ex.Message;
					}
				}
				else
				{
		...
		{
			if (this.SQLiteCon != null && this.SQLiteCon.State == ConnectionState.Open)
			{
				this.SQLiteCon.Dispose();
				if (this.SQLiteCmd != null)
					this.SQLiteCmd.Dispose();
				this.SQLiteCon.Close();
				base.msg = "最后->关闭 SQLite，是一款轻型的数据库公司 要请继续调用对象Open() 方法连接数据库";
				return true;
			}
			return false;
		}
		...
		public override DataTable GetTable(string getsWhere, List<string> keys, List<object> values)
		{
			...
			if (this.IsOpenDataBase)
			{
				try
				{
					...
					SQLiteDataReader sqr = sqlcmd.ExecuteReader();

					dtData.Load(sqr);

					if (dtData != null && dtData.Rows.Count > 0)
					{
						base.msg = "第三步 SQLite，是一款轻型的数据库公司读取成数据成功 sql语句：" + sql + "读取行数" + dtData.Rows.Count;
					}
					else
					{
						base.msg = "第三步 SQLite，是一款轻型的数据库公司读取数据成功 sql语句：" + sql + "读取行数" + dtData.Rows.Count;
						;
					}
				}
				catch (Exception ex)
		...
		public override long Del(string delWhere)
		{
...

			{
				try
				{
					...
					base.msg = "第二步 SQLite，是一款轻型的数据库公司创建/删除数据sql 语句成功 sql语句：" + sql + " 删除行数：" + icounter;
				}
				catch (Exception ex)
				{
					base.msg = "第二步 SQLite，是一款轻型的数据库公司创建/增加数据sql 语句失败 sql语句：" + sql + "  异常信息：" + ex.Message;
