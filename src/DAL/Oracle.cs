...
namespace DAL
{
	...
	{
		public static string OracleConfigString { get { return "dbOracle"; } }

		public Oracle(string TableName)
			: base(Oracle.OracleConfigString, TableName)
		{
		}

		public Oracle(string ConfigString, string TableName)
			: base(ConfigString, TableName)
		{
		}

		public OracleCommand MsSqlCmd { get; set; }
		...
		public override bool Add(System.Data.DataTable addTable)
		{
			...
			{
				OracleCommand sqlcmd = this.OracleCon.CreateCommand();
		...
		{
			if (this.OracleCon != null && this.OracleCon.State == ConnectionState.Open)
			{
		...
		{
			this.OracleCon = new OracleConnection(sql);
			if (this.OracleCon != null && OracleCon.State == ConnectionState.Closed)
			{
				this.OracleCon.Open();
		...
		public override bool Open()
		{
			try
			{
				this.OracleCon = new OracleConnection(base.ConfigStr);
				if (this.OracleCon != null && OracleCon.State == ConnectionState.Closed)
				{
					this.OracleCon.Open();
			...
			if (this.IsOpenDataBase)
			{
				try
				{
					...
					if (keys != null)
						...
						{
							sqlcmd.Parameters.Add(keys[i], values[i]);
						}

					OracleDataReader sqr = sqlcmd.ExecuteReader();
		...
		public override long Del(List<long> delListID)
		{
...

			{
				try
				{
					...
					foreach (string _key in base.listSqlParam.Keys)
					{
						sqlcmd.Parameters.Add(_key, base.listSqlParam[_key]);
					}
